...
def check_k8s_standards_compliance():
    """Check compliance with all k8s standards (Rules 01-05)"""
    ...
    if not liveness_probe:
        issues.append("Health Probes: Missing liveness probe")
    else:
        liveness_path = liveness_probe.get('httpGet', {}).get('path', '')
        if liveness_path != '/actuator/health/liveness':
            issues.append("Health Probes: Incorrect liveness probe path")
    
    if not readiness_probe:
        issues.append("Health Probes: Missing readiness probe")
    else:
        readiness_path = readiness_probe.get('httpGet', {}).get('path', '')
        if readiness_path != '/actuator/health/readiness':
            issues.append("Health Probes: Incorrect readiness probe path")
    ...
    if issues:
        ...
        print("✓ All K8s Standards (Rules 01-05) are compliant!")
        return True
